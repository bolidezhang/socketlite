...
SL_Crypto_CRC64::SL_Crypto_CRC64()
    : checksum_(0)
{
}
...
int SL_Crypto_CRC64::checksum_len()
{
    return sizeof(uint64);
}

int SL_Crypto_CRC64::init()
{
    checksum_ = 0;
    return 0;
}

int SL_Crypto_CRC64::update(const unsigned char *data, uint len)
{
    checksum_ = checksum(data, len, checksum_);
    return 0;
}

int SL_Crypto_CRC64::final(unsigned char *checksum, uint len)
{
...
int SL_Crypto_CRC64::final(const unsigned char *in, uint in_len, unsigned char *out, uint out_len)
{
...
uint64 SL_Crypto_CRC64::checksum(const unsigned char *data, uint len, uint64 sum)
{
    assert(data && (len > 0));

    for (uint i=0; i<len; ++i)
    {
        sum = CRC64_TABLE[(uint8)sum ^ data[i]] ^ (sum >> 8);
    }
    return sum;
}
